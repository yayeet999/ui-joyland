...
const FormCard: React.FC<FormCardProps> = ({
  ...
  return <motion.div initial={{
    ...
    duration: 0.4,
    delay: delay * 0.1
  }} className={`bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden ${isAuthForm ? 'col-span-1 md:w-full lg:w-full' : 'col-span-1'}`}>
      {/* Component Preview */}
      <div className="relative">
        ...
        <div className={`relative ${isAuthForm ? 'aspect-[16/12] md:aspect-[16/11] lg:aspect-[16/12]' : 'min-h-[200px]'} bg-gray-50 dark:bg-gray-900`}>
          <div className="absolute inset-0 flex items-center justify-center p-6 md:p-4 lg:p-4 overflow-auto px-[4px] py-[24px]">
            <div className={`w-full ${isAuthForm ? 'max-w-full md:max-w-full lg:max-w-full transform scale-95 md:scale-100' : ''}`}>
              {renderFormPreview()}
            </div>
          </div>
        </div>
